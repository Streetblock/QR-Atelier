...
class QRPlaygroundApp {
  ...
  #buildSvg(size) {
    if (this.state.options.codeType === 'microqr') {
      const microGenerator = new MicroQrCore(this.state.data, {
      ...
      const microMatrix = microGenerator.generate()
      const microRenderer = new MicroQrSvgRenderer(microMatrix, {
        size,
        margin: 8,
  ...
  #syncControlStates() {
    const isMicro = this.state.options.codeType === 'microqr'
    this.ui.microOptions.classList.toggle('hidden', !isMicro)
    this.ui.cornerShape.disabled = isMicro
    this.ui.logoUpload.disabled = isMicro
    this.ui.clearLogoBtn.disabled = isMicro
  ...
  #getFilePrefix() {
    return this.state.options.codeType === 'microqr' ? 'micro-qr-code' : 'qr-code'
  }
