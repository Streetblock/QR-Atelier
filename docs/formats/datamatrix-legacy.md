...
Public patents and independently captured complete symbols are useful corroborating sources, but are not complete enough to replace those normative annexes.
...
- ISO/IEC 16022:2024 removed the complete historical ECC 000-140 family.
...
6. Compare every level with independently obtained complete-symbol vectors and physical output.
7. Expose the independently verified core through a stable browser and Node.js API; application-specific integrations remain outside this repository.

## Acceptance criteria

- The default `ecc: 0` option and explicit `ecc: 0` produce the same symbol.
...
- Too-small forced sizes are rejected; explicit dimensions must be supported odd square sizes from 9 through 49.
- The Annex Q ECC 050 example is module-exact.
- Independent complete-symbol and physical-output comparisons are recorded alongside their exact encoder options.
...
- Independently captured physical symbols for complete-matrix verification.
