...
test('registers QR and Micro QR with isolated defaults and capabilities', () => {
  assert.deepEqual(formatRegistry.list().map((format) => format.id), ['qr', 'microqr'])
...
test('creates renderable QR SVG output through the registry interface', () => {
  ...
  const microRenderer = formatRegistry.createRenderer('microqr', {
    payload: '12345',
    size: 256,
    options,
  })
  assert.match(microRenderer.render(), /^<svg\b/)
})
