// ==========================================
// DmSvg.js - SVG Renderer for Data Matrix modules
// ==========================================

export class DmSvgRenderer {
  ...
  constructor(dmDataResult, options = {}) {
    if (!dmDataResult || !dmDataResult.modules) {
      throw new Error('Valid Data Matrix result from DmCore is required.')
    }
    this.dmCode = dmDataResult
    this.style = { ...DmSvgRenderer.DEFAULT_STYLE, ...options }
  }

  render() {
    const modules = this.dmCode.modules
    const rows = this.dmCode.rows
    const cols = this.dmCode.cols
    const viewBoxWidth = cols + this.style.margin * 2
    const viewBoxHeight = rows + this.style.margin * 2
    const gradientId = `dm-gradient-${Math.random().toString(36).slice(2, 10)}`
    ...
    return [
      `<svg xmlns="http://www.w3.org/2000/svg" width="${this.style.size}" height="${this.style.size}" viewBox="0 0 ${viewBoxWidth} ${viewBoxHeight}" fill="none" role="img" aria-label="Data Matrix code">`,
      `<defs>`,
      ...
      `</linearGradient>`,
      `</defs>`,
      `<rect width="${viewBoxWidth}" height="${viewBoxHeight}" fill="${this.style.background}" rx="2.4"/>`,
